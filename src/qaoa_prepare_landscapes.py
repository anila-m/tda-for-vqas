...
def generate_LHS_sample_points_for_qaoa(number_of_samples=10000, min_gamma=-np.pi, max_gamma=np.pi, min_beta=-np.pi/2, max_beta=np.pi/2):
    """
    Generates LHS sample points for QAOA for p=1,2,3 (i.e. 2D, 4D and 6D sample points)
    ...
    """
    # determine correct lowerleft and upper right corner if not 
    ...
    for p in range(1,4):
        ...
        if max_beta==np.pi/2:
            beta_value = "0.5pi"
        file_name = f"samples_LHS_{number_of_samples}_gamma_{gamma_value}_beta_{beta_value}.json" 
...
def main():
    ...
    # TODO: diesen Teil parallelisieren
    id=0
    config_dict = {}
    ...
    for p in range(1,4):
        for num_qubits in [3,6,9,12,15,18]:
            ...
            with ProcessPoolExecutor(max_workers=5) as exe:
                futures = [exe.submit(generate_qaoa_landscape,ham_config_dict, f"ham_MAXCUT_weighted_-10_to_10_qubits_{num_qubits}_run_{run}", p) for run in range(num_runs)]            
                # await results & save them:
                for future in as_completed(futures):
                    id, samples_file_name, elapsed_time, landscape, ham_file_label = future.result()
                    ...
                    landscape_file_name = f"landscape_qaoa_{id}_qubits_{num_qubits}_run_{run}_p_{p}_timestamp"
...
def generate_qaoa_landscape(ham_config_dict, ham_file_label, p):
    print("started", ham_file_label)
    n=2*p
    num_qubits = ham_config_dict[ham_file_label]["num_qubits"]
    run = ham_config_dict[ham_file_label]["run"]
    id = (p-1)*30+num_qubits//3*run # compute config id for later
    ...
    s = f"samples_LHS_10000_gamma_pi_beta_0.5pi.json" 
    samples_file_name = f"resources/sample_points/QAOA/{n}D/{s}" 
    samples_directory = os.path.join(
                os.path.join(os.getcwd(), samples_file_name)
            )
    with open(samples_directory) as f:
    ...
    return id, samples_file_name, elapsed_time, landscape, ham_file_label
