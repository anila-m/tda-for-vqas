...
def create_plot(
    ...
    print(scan2D_result.values.shape)
    if remove_constant:
...
def create_tda_plot(
    ...
    plot_scans(fig, ax1, scan2D_result, fix_x_y_extents, unit)
    print("[INFO] plot scans done")
    #plot_fourier(fig, ax2, fourier_result, fourier_res_x, fourier_res_y)
    max_dim = 1
    plot_persistence(fig, ax2, scan2D_result, filelabel=label, max_dim=max_dim, timestamp_str=timestamp_str)
    print("[INFO] plot persistence done")
    if color_bar_font_size is not None:
        adjust_color_bar(ax1, ax2, color_bar_font_size)
...
def remove_constant_term(
    ...
    print("constant term sign", constant_term_sign)
...
def plot_persistence(
    ...
    if timestamp_str is None:
        timestamp_str = generate_timestamp_str()
    directory_name = os.path.join(os.getcwd(), f"results/persistence_plots_{timestamp_str}")
    if not os.path.exists(directory_name):
        os.mkdir(directory_name)
        print(f"Created directory {directory_name}")
    start = datetime.now()
    cost_landscape = np.vstack(np.dstack((scan2D_result.params_grid, scan2D_result.values)))
    diagrams = ripser(cost_landscape, maxdim=max_dim)['dgms']
    filename = f"{filelabel}_persistence_diagram.npy"
    file_directory = os.path.join(directory_name, filename)
    #np.save(file_directory, diagrams)
    elapsed_time = datetime.now()-start
    print("Persistence Plot: elapsed time", elapsed_time)
    #title=f"QAOA {filelabel}"
    plot_diagrams(diagrams, show=False, ax=ax) # axis limits fit transformed loss values (to [50,100])
